  ...
  return (
    <Joyride
      ...
      tooltipComponent={(props) => (
        <div className={`bg-white rounded ${props.index === 0 ? "p-10 max-w-[650px]" : "p-4 max-w-[320px]"}`}>
          <div className="flex justify-between items-center">
            <div
              className={`font-semibold ${props.index === 0 ? "text-[22px] leading-[30px]" : "text-base leading-6"}`}
            >
              {props.step.title}
            </div>
          ...
          <div className={`flex justify-between items-center ${props.index === 0 ? "mt-10" : "mt-6"}`}>
            {props.index !== 0 && (
              <p className="text-[#767676] text-sm">
                Step {props.index} of {props.size}
              </p>
            )}
