...
export function useDepositionTomoRunCounts(
  depositionId: number | undefined,
  datasetIds: number[],
) {
  return useQuery({
    queryKey: ['deposition-tomo-run-counts', depositionId, datasetIds],
    queryFn: async (): Promise<RunCountsResponse> => {
      ...
      const response = await fetch(
        `/api/deposition-tomo-run-counts?depositionId=${depositionId}&datasetIds=${datasetIds.join(
          ',',
        )}`,
      )

      if (!response.ok) {
        throw new Error('Failed to fetch tomogram run counts')
      }
