...
| **SDS Tailwind Tokens** | Design tokens via Tailwind | Colors, typography, spacing - **always prefer SDS tokens** |
| **Tailwind CSS**        | Utility-first styling      | Layout (flex, grid) and utilities not covered by SDS       |
| **CSS Modules**         | Component-scoped styles    | Complex animations, MUI overrides, pseudo-selectors        |
| **Material-UI**         | Complex components         | **Last resort** - only when SDS doesn't provide equivalent |
...
```
┌─────────────────────────────────────────┐
│ 1. SDS Components (Primary)             │  ← Start here for UI
├─────────────────────────────────────────┤
│ 2. SDS Tailwind Tokens (Styling)        │  ← Colors, typography, spacing
├─────────────────────────────────────────┤
│ 3. Tailwind Utilities (Layout)          │  ← Flex, grid, positioning
├─────────────────────────────────────────┤
│ 4. CSS Modules (Complex styles)         │  ← Animations, MUI overrides
├─────────────────────────────────────────┤
│ 5. Material-UI (Fallback)               │  ← Only when SDS lacks it
└─────────────────────────────────────────┘
```

**Philosophy:** Always prefer SDS components and tokens first. Use Tailwind for layout utilities. Resort to CSS Modules for complex styles and MUI only when SDS doesn't provide an equivalent.
...
│  └─ Check SDS components first → @czi-sds/components
│     └─ SDS has it? → Use SDS component
│     └─ SDS doesn't have it? → Use MUI as fallback
│
├─ Styling any element?
│  └─ Use SDS Tailwind tokens for:
│     • Colors → light-sds-color-semantic-* or light-sds-color-primitive-*
│     • Typography → text-sds-body-*, text-sds-header-*, etc.
│     • Spacing → p-sds-*, m-sds-*, gap-sds-*
│     • Borders → rounded-sds-*, border-light-sds-color-*
│     • Shadows → shadow-sds-*
│
├─ Layout (flex, grid, positioning)?
│  └─ Use Tailwind utilities (flex, grid, absolute, etc.)
│     └─ Use SDS spacing for gaps: gap-sds-m, not gap-4
│
├─ Complex animations or pseudo-selectors?
│  └─ Use CSS Modules with @apply for SDS tokens
│
└─ Targeting MUI classes for overrides?
   └─ Use CSS Modules with :global()
```
...
- **Always check SDS components first** before using MUI
...
- Use MUI when an SDS component exists
...
### Material-UI (Fallback Only)

✅ **Do:**

- **Check SDS first** - only use MUI when SDS doesn't have the component
- Style MUI components with SDS Tailwind tokens
...
- [GraphQL Integration](./03-graphql-integration.md) - Data fetching for styled components
- [State Management](./04-state-management.md) - Managing UI state
- [Feature Flags](./06-feature-flags.md) - Conditional styling based on environment
