...
| Server State | Remix loaders | Data from GraphQL API | Covered in [GraphQL Integration](./03-graphql-integration.md) |
...
1. **URL is the source of truth** for filters, pagination, and navigation
2. **Jotai atoms** manage ephemeral UI state (modals, selections, history)
3. **React Context** provides global configuration and utilities
4. **Remix loaders** handle server state (no client-side cache)
...
The application uses Jotai for lightweight, atomic state management.
...
- [GraphQL Integration](./03-graphql-integration.md) - Server state management with Apollo
- [Styling System](./05-styling-system.md) - UI state and styling approaches
- [Feature Flags](./06-feature-flags.md) - Environment-based state configuration
