...
The custom `useQueryParam` hook provides React state-like API for URL parameters:

```typescript
// app/hooks/useQueryParam.ts
export function useQueryParam<T>(
  queryParam: QueryParams,
  { defaultValue, deserialize, serialize, preventScrollReset } = {},
  ...
  const setValue = useCallback<QueryParamStateSetter<T>>(
    (nextValue) =>
      setSearchParams((prev) => {
        ...
        if (nextValue) {
          prev.set(queryParam, serialize(nextValue))
        }
        return prev
      }, { preventScrollReset }),
...
The `shouldRevalidatePage` utility checks if relevant parameters changed:

```typescript
// app/utils/revalidate.ts
export function shouldRevalidatePage({
  ...
  if (
    ...
    return false  // No relevant params changed, skip refetch
  }
...
### 2. Server-Side Utility Routes

Some API routes serve different purposes:
...
- [GraphQL Integration](./03-graphql-integration.md) - Apollo Client, codegen, queries
- [State Management](./04-state-management.md) - Jotai atoms, URL state, contexts
