...
PR titles **must** follow conventional commits format. This is enforced by CI.
...
| ---------- | ----------------------------- | ---------------------------------------------------- |
| `feat`     | New feature                   | `feat: add tomogram filtering by resolution`         |
| `fix`      | Bug fix                       | `fix: resolve incorrect dataset count display`       |
| `perf`     | Performance improvement       | `perf: optimize table rendering with virtualization` |
| `refactor` | Code restructuring            | `refactor: extract dataset logic into custom hook`   |
| `style`    | Code style changes (no logic) | `style: fix indentation in DatasetTable`             |
| `test`     | Test additions/changes        | `test: add E2E tests for viewer page`                |
| `docs`     | Documentation                 | `docs: update GraphQL integration guide`             |
| `build`    | Build system changes          | `build: update Remix to v2.17`                       |
| `ci`       | CI/CD changes                 | `ci: add E2E test caching`                           |
| `chore`    | Maintenance tasks             | `chore: update dependencies`                         |
| `revert`   | Revert previous commit        | `revert: undo experimental filter changes`           |
...
```
feat: add deposition filtering by author
fix: correct tomogram count in dataset details
perf: split ESLint config to optimize VS Code performance
docs(architecture): add GraphQL integration guide
test(e2e): add viewer page screenshot tests
```
...
refactor!: rename dataset status field to state
```
...
git checkout -b feat/author-filtering
git checkout -b fix/dataset-count-bug
